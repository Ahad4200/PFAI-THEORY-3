...
# -------------------------
model = Chain(
    Dense(4, 32, relu),       # Increased neurons
...
opt_state = Flux.setup(Adam(0.001), model) # Adjusted learning rate
...
train_loss = Float64[]
train_acc = Float64[]
test_acc = Float64[]  # To track test accuracy during training
...
for epoch in 1:epochs
    ...
    ta = accuracy(hcat(first.(test_data)...), hcat(last.(test_data)...))  # Test accuracy
    push!(train_loss, l)
    push!(train_acc, a)
    push!(test_acc, ta)
    println("Epoch $epoch - Loss: $(round(l, digits=4)) | Train Accuracy: $(round(a * 100, digits=2))% | Test Accuracy: $(round(ta * 100, digits=2))%")
end
...
println("Final Test Accuracy: ", round(accuracy(test_X, test_Y) * 100, digits=2), "%")
...
plot(test_acc, xlabel="Epoch", ylabel="Accuracy", title="Test Accuracy", legend=false)
